...
async def build_kb(db, files: list[Path], top_k: int):
    """在已打开的 session 上 ingest，返回 KB（勿对 get_db 提前 return）。"""
    from app.rag.retriever import KnowledgeBase

    kb = KnowledgeBase(db)
    for f in files:
        n = await kb.ingest_file(f, title=f.name)
        print(f"  导入 {f.name}: {n} 块")
    print(f"  知识库共 {kb.chunk_count} 块 | top_k={top_k}")
    return kb


async def run_rag(kb, question: str, top_k: int) -> tuple[str, list[str]]:
    """检索 -> 生成，返回 (回答, 检索上下文列表)。"""
    from app.rag.retriever import RAG_PROMPT, build_llm
    from app.services.resilience import ainvoke_with_retry

    chunks = await kb.search(question, top_k=top_k)
...
def write_report(report: dict, out: Path) -> None:
    ...
    lines.append(
        f"- LLM: {report['meta']['llm_model']} | "
        f"Embedding: {report['meta']['embedding_model']} | "
    ...
    if worst:
        ...
        for w in worst:
            lines.append(
                f"- **{w['id']}** {w['category']}：Judge {w['judge_score']} 分 - {w['question']}"
            )
...
async def async_main(args) -> None:
    from app.database import AsyncSession, engine, init_db
    from app.rag.retriever import build_llm
    ...
    try:
        async with AsyncSession() as db:
            kb = await build_kb(db, files, args.top_k)
            await db.commit()
            ...
            for i, item in enumerate(items, 1):
                ...
                answer, contexts = await run_rag(kb, question, args.top_k)
        ...
        # 进程退出前归还连接池，避免 greenlet is being finalized
    ...
    buckets = [
        {"bucket": b, "count": sum(1 for s in judge_scores if b <= s < b + 1)}
        for b in range(1, 6)
    ]
    ...
    cat_summary = {
        k: {"count": len(v), "judge_avg": round(sum(v) / len(v), 2)}
            ...
            "worst_cases": [
                {
                    ...
                    "category": w["category"],
                    "question": w["question"],
                    "judge_score": w["judge_score"],
                    "judge_reason": w["judge_reason"],
                }
                for w in worst
            ],
        },
    ...
    out = (
        Path(args.out)
        if args.out
        else BASE_DIR / "eval" / "reports" / f"report_{time.strftime('%Y%m%d_%H%M%S')}.json"
    )
    ...
    if ragas:
        for k in ("faithfulness", "answer_relevancy", "context_precision", "context_recall"):
            print(f"| {k} | {avg.get(k) if avg.get(k) is not None else '-'} |")
